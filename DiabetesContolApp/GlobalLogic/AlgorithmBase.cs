...
namespace DiabetesContolApp.GlobalLogic
{
    ...
    {
        public const int MINIMUM_OCCURENCES = 10;
        public const double LOWER_BOUND_FOR_PREDICTION_INTERVAL = -1.0;
        public const double ABSOLUTE_MAXIMUM_DISTANCE_CHANGE = 1.0;
        ...
        public static IGroceryService _groceryService;
        ...
        async public static Task<bool> RunStatisticsOnReminder(ReminderModel currentReminder)
        {
            try
            {
                ...
                if (logs == null)
                {
                    reminder.Logs.ForEach(log => log.GlucoseAfterMeal = null); //Set the data invalid
                    reminder.GlucoseAfterMeal = -1.0f;
        ...
        async public static Task<List<LogModel>> PartitionGlucoseErrorToLogs(ReminderModel reminder)
        {
            try
            {
        ...
        async public static Task<bool> UpdateScalarValues(List<LogModel> logs)
        {
        ...
        /// <returns>True if no errors, else false</returns>
        async public static Task<bool> UpdateDayProfileScalars(int dayProfileID)
        {
            try
            {
        ...
        public static DataPoint GetDataPointForDayProfileCarbScalar(LogModel log)
        {
            try
            {
        ...
        public static DataPoint GetDataPointForDayProfileGlucoseScalar(LogModel log)
        {
            try
            {
                if (log.DayProfile == null || log.DayProfile.DayProfileID < 0)
        ...
        /// </summary>
        /// <returns>True if no errors, else false.</returns>
        async public static Task<bool> UpdateCorrectionInsulinScalar()
        {
            try
            {
                _scalarService ??= ScalarService.GetScalarService();
        ...
        /// </returns>
        public static DataPoint GetDataPointForCorrectionInsulin(LogModel log)
        {
            try
            {
                if (!log.IsLogDataValid())
        ...
        /// <returns>True if no errors, else false.</returns>
        async public static Task<bool> UpdateGroceryScalar(int groceryID)
        {
            try
            {
                _groceryService ??= GroceryService.GetGroceryService();
                //Get current grocery
                GroceryModel currentGrocery = await _groceryService.GetGroceryAsync(groceryID);

                if (currentGrocery == null)
                    throw new NullReferenceException("There is no grocery with the given ID: " + groceryID);
                ...
                if (dataPointsForGrocery.Count >= MINIMUM_OCCURENCES)
                {
                    ...
                    if (await _applicationProperties.DisplayAlert("Changing grocery (" + currentGrocery.Name + ") carb scalar", currentGrocery.CarbScalar + " => " + currentGrocery.CarbScalar * scalingFactor, "OK", "Cancel"))
                    {
                        ...
                        await _groceryService.UpdateGroceryAsync(currentGrocery);
        ...
        /// </returns>
        public static DataPoint GetDataPointForGrocery(LogModel log, int groceryID)
        {
            try
            {
                if (!log.IsLogDataValid())
        ...
        public static double GetGreatestSafeDistanceFromWantedLine(List<DataPoint> dataPoints)
        {
            ...
            //Adjust the x-values to be on the scale of the y-values to
            //avoid swamping issues when calculating to a number of precise digits.
